    ...
    for j in range(num_skels):
        ...
        for k in range(config.NUM_KP):
            if skeletons[j][k,2] == 0:
                continue
            p = np.sqrt(np.square(features[p_i,p_j,2*k:2*k+2] - np.array([[ skeletons[j][k,0], skeletons[j][k,1] ]])).sum(axis=-1))
            p *= skeletons[j][k,2] / scale
            this_prob += p
            norm_factor += skeletons[j][k,2]
        probs[:, j] = this_prob / norm_factor
        
    P = np.zeros(map_shape+(num_skels,))
    P[p_i, p_j, :] = probs
    masks = np.zeros(map_shape+(num_skels,))
    masks[idx[:,:,1].flatten(), idx[:,:,0].flatten(), P.argmax(axis=-1).flatten()] = 1
    masks[P.max(axis=-1) < config.INSTANCE_SEG_THRESH, :] = 0
